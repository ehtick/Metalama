...
partial class MetaSyntaxRewriter
{
	...
	protected virtual ExpressionSyntax TransformAllowsConstraintClause( AllowsConstraintClauseSyntax node )
	{
		...
		result = InvocationExpression(this.MetaSyntaxFactory.SyntaxFactoryMethod(nameof(AllowsConstraintClause))).WithArgumentList(ArgumentList(SeparatedList<ArgumentSyntax>(new SyntaxNodeOrToken[]{
			Argument(this.Transform(node.AllowsKeyword)).WithLeadingTrivia(this.GetIndentation()),
			Token(SyntaxKind.CommaToken).WithTrailingTrivia(GetLineBreak()),
			Argument(this.Transform(node.Constraints)).WithLeadingTrivia(this.GetIndentation()),
		})));
		this.Unindent();
	...
	protected virtual ExpressionSyntax TransformRefStructConstraint( RefStructConstraintSyntax node )
	{
		...
		result = InvocationExpression(this.MetaSyntaxFactory.SyntaxFactoryMethod(nameof(RefStructConstraint))).WithArgumentList(ArgumentList(SeparatedList<ArgumentSyntax>(new SyntaxNodeOrToken[]{
			Argument(this.Transform(node.RefKeyword)).WithLeadingTrivia(this.GetIndentation()),
			Token(SyntaxKind.CommaToken).WithTrailingTrivia(GetLineBreak()),
			Argument(this.Transform(node.StructKeyword)).WithLeadingTrivia(this.GetIndentation()),
		})));
		this.Unindent();
		return result;
	}
